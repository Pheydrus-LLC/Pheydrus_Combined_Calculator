...
function getMirrorLineHtml(item: { planet?: string; house?: number; section?: string }, goalShort: string): string {
 ...
 'Saturn-5': `You build the offer, get excited, draft the posts - and pull back right before you publish. Every time. The same wall appears in romance: you open up enough, then go quiet - not from lack of feeling, but from fear of being truly seen.`,
 ...
 'Pluto-6': `Are you stuck in performative busyness - doing work that feels productive but isn't moving the needle toward ${esc(goalShort)}?`,
 ...
 'EnvUranus-2': `Does your income feel erratic - breakthrough months followed by drought - while ${esc(goalShort)} stays out of reach?`,
 ...
 if (!text) return '';
 const styled = applyKmsStyle(text);
 return `<p style="margin:0 0 6px;font-size:12px;font-style:italic;color:#1C1A2E;line-height:1.55;font-family:${CORMORANT};">&ldquo;${styled}&rdquo;</p>`;
}
...
<div style="background:#F5F1EB;padding:40px 48px;min-height:980px;color:#1C1A2E;">
 ...
 <p style="margin:0;font-size:14px;font-style:italic;color:#7A5A1A;line-height:1.75;font-family:${CORMORANT};">You did the mindset work, the strategy work, and the coaching. Results still stall at the same point. The missing variable is energetic structure. Thinking harder does not solve this layer. You already have the capacity. This report shows the sequence to unlock it.</p>
 </div>
...
<div style="background:#F5F1EB;padding:40px 48px;min-height:980px;color:#1C1A2E;">
 ...
 <div style="border-left:4px solid #C9A84C;padding:12px 20px;margin-bottom:24px;background:#FDFBF6;">
	<p style="margin:0;font-size:14px;font-style:italic;color:#7A5A1A;line-height:1.7;font-family:${CORMORANT};">"You did the degree, the career, and the inner work. Results still stall at the same point. This pressure pattern is the pre-upgrade signal this report maps."</p>
 </div>

 <!-- Body -->
 <p style="margin:0 0 12px;font-size:13px;color:#444;line-height:1.8;font-family:${INTER};font-weight:700;">You are in an identity shift.</p>
 <p style="margin:0 0 12px;font-size:13px;color:#444;line-height:1.8;font-family:${INTER};">These patterns are the exact conditions that precede a major identity upgrade. Three invisible forces are pulling against each other, and that friction marks the edge where your previous identity loses control and your next identity takes over.</p>
 <p style="margin:0 0 12px;font-size:14px;font-weight:700;color:#C9A84C;line-height:1.6;font-family:${INTER};">Your identity upgrade is around the corner, and the first signals are active now.</p>
 <p style="margin:0 0 28px;font-size:13px;color:#444;line-height:1.8;font-family:${INTER};">You now choose how you enter this window: with a map and deliberate action, or by repeating the same loop.</p>
