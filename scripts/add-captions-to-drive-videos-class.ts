...
const RAW_ARG = args[0] || process.env.CAPTION_DRIVE_FOLDER_ID;
if (!RAW_ARG) {
  console.error('Usage: npx tsx scripts/add-captions-to-drive-videos.ts <drive_folder_id_or_file_link> [--output-folder FOLDERNAME] [--caption-style STYLE] [--preserve-filename]');
  process.exit(1);
}
...
for (let i = 1; i < args.length; i++) {
  if (args[i] === '--output-folder' && args[i+1]) {
    ...
    i++;
  } else if (args[i] === '--caption-style' && args[i+1]) {
    CUSTOM_CAPTION_STYLE = args[i+1];
    i++;
  } else if (args[i] === '--preserve-filename') {
...
function muxSubtitle(
  ...
  execSync(
    `"${ffmpeg}" -y -i "${videoPath}" -vf "subtitles='${escapedSrt}':force_style='${style}'" -c:a copy "${outputPath}"`,
    { stdio: 'pipe' }
  );
}
...
async function ensureSubfolder(drive: ReturnType<typeof buildDriveClient>, parentId: string, folderName: string): Promise<string> {
...
async function main(): Promise<void> {
  ...
  const { files: videos, parentFolderId: origParentFolderId } = await resolveFiles(drive);
  let parentFolderId = origParentFolderId;
  if (OUTPUT_FOLDER) {
    parentFolderId = await ensureSubfolder(drive, origParentFolderId, OUTPUT_FOLDER);
    console.log(`Using subfolder: ${OUTPUT_FOLDER} (${parentFolderId})`);
  }
